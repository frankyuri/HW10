...
int BinaryNodeTree<ItemType>::getHighestFullLevelHelper(BinaryNode<ItemType>* subTreePtr) const
{
    ...
    if (subTreePtr->getLeftChildPtr()==nullptr && subTreePtr->getRightChildPtr()==nullptr)
        return 1;
    if (subTreePtr->getLeftChildPtr() != nullptr && subTreePtr->getRightChildPtr() != nullptr) {
        int l = getHighestFullLevelHelper(subTreePtr->getLeftChildPtr() ),
            r = getHighestFullLevelHelper(subTreePtr->getRightChildPtr() );
        if (l == r)
            return 1 + r;
        else {
            if (l > r)
...
int BinaryNodeTree<ItemType>::getMaximumHelper(BinaryNode<ItemType>* subTreePtr) const
{
    ...
    if (subTreePtr->getRightChildPtr()) {
        r = getMaximumHelper(subTreePtr->getRightChildPtr());
        if (r < subTreePtr->getItem())
            r = subTreePtr->getItem();
    }
    else
        r = INT_MIN;
    if (subTreePtr->getLeftChildPtr()) {
        l = getMaximumHelper(subTreePtr->getLeftChildPtr());
        if (l < subTreePtr->getItem())
            l = subTreePtr->getItem();
    }  
    else
        l = INT_MIN;
    if (l > r)
...
int BinaryNodeTree<ItemType>::getNumberOfNodesHelper(BinaryNode<ItemType>* subTreePtr) const
{
    ...
    if (subTreePtr->getLeftChildPtr() != nullptr)
        l = getNumberOfNodesHelper(subTreePtr->getLeftChildPtr());
    if (subTreePtr->getRightChildPtr() != nullptr)
        r = getNumberOfNodesHelper(subTreePtr->getRightChildPtr());
    return 1 + l + r;
